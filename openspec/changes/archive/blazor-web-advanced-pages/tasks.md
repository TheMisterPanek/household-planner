...
- [x] 11.1 Navigate to `/meals` → existing meals appear in sidebar; click one → ingredients and steps load in detail panel
- [x] 11.2 Add a new meal → it appears in the sidebar; add an ingredient and a step → confirm via `/meals` command in Telegram
- [x] 11.3 Rename a meal → name updates in sidebar and detail; confirm in Telegram
- [x] 11.4 Reorder steps with ↑/↓ → order persists after page refresh
- [x] 11.5 Delete a meal → removed from list; Telegram `/meals` no longer shows it
- [x] 11.6 Navigate to `/week` → Mon–Sun grid renders; assign a meal to Wednesday via dropdown → confirm via Telegram `/week`
- [x] 11.7 Clear Wednesday assignment → "— Not set —" shown; confirmed in Telegram
- [x] 11.8 Navigate to `/settings` → current language shows; change to Polish → Save → subsequent Telegram bot replies come in Polish
- [x] 11.9 Change expiry threshold → Save → success message shown; threshold persists after page refresh
- [x] 11.10 Navigate to `/ai` → type a query → bot response appears in chat bubbles
- [x] 11.11 Submit without `OPENROUTER_API_KEY` (remove from env) → error banner shown, page does not crash
- [x] 11.12 All existing bot commands unaffected
