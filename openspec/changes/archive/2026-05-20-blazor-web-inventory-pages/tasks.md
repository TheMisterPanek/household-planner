...
- [x] 1.1 Check if `GetPageAsync(groupId, page, pageSize, nameFilter, dateFrom, dateTo)` already exists
- [x] 1.2 If not, add the method:
...
- [x] 2.1 Register `ShoppingItemRepository` as `AddScoped`
- [x] 2.2 Register `PurchaseHistoryRepository` as `AddScoped`
- [x] 2.3 Register `PriceLogRepository` as `AddScoped`
- [x] 2.4 Register `ShoppingListService` as `AddScoped`

---

## 3. `/list` — Shopping List Page

- [x] 3.1 Create `Components/Pages/ShoppingList.razor` inheriting `AuthenticatedPageBase`:
...
- [x] 3.2 Add item form (collapsible):
...
- [x] 3.5 Delete with inline confirmation:
...
- [x] 4.1 Page redirects to `/login` when `WebSessionStore.IsAuthenticated` is false
- [x] 4.2 Page renders item rows from `ShoppingItemRepository.GetActiveAsync`
- [x] 4.3 Adding an item calls `ShoppingListService.AddItemAsync` with correct parameters
- [x] 4.4 Clicking ✎ on a row renders input fields for that row only
- [x] 4.5 Save on edit calls `ShoppingItemRepository.UpdateAsync` with modified values
- [x] 4.6 Clicking ✕ shows inline confirmation; clicking Yes calls `DeleteAsync`
- [x] 4.7 Checking the checkbox calls `MarkDoneAsync`

---

## 5. `/history` — Purchase History Page

- [x] 5.1 Create `Components/Pages/History.razor` inheriting `AuthenticatedPageBase`:
...
- [x] 6.1 Renders table rows from `GetPageAsync` result
- [x] 6.2 Prev button disabled on page 1; Next button disabled when on last page
- [x] 6.3 Changing name filter triggers reload with `_page` reset to 1

---

## 7. `/prices` — Price Log Page

- [x] 7.1 Create `Components/Pages/Prices.razor` inheriting `AuthenticatedPageBase`:
...
- [x] 8.1 Groups price entries by item name correctly
- [x] 8.2 Trend arrow is ↑ when last price > previous; ↓ when lower; → when equal or single entry
- [x] 8.3 Clicking a card sets `_expandedItem`; clicking again clears it

---

## 9. NavMenu Update

- [x] 9.1 Ensure nav links for `/list`, `/history`, `/prices` in `NavMenu.razor` point to the correct routes (they were placeholder links in the scaffold — verify they now load the real components)

---

## 10. Smoke Test (manual — do not mark complete without user confirmation)

- [x] 10.1 Navigate to `/list` → shopping list loads with existing items from DB
- [x] 10.2 Click [+ Add item] → form opens; fill in Name + Qty; submit → item appears in list; bot `/list` also shows it
- [x] 10.3 Check checkbox on an item → item grays out; `/list` in Telegram confirms it's done
- [x] 10.4 Click ✎ on an item → row becomes editable; change name; Save → updated name persists after page refresh
- [x] 10.5 Click ✕ → confirmation appears; click Yes → item gone; confirmed via Telegram `/list`
- [x] 10.6 Navigate to `/history` → purchase history table loads; filter by item name → rows narrow correctly; Prev/Next pagination works
- [x] 10.7 Navigate to `/prices` → price cards appear; trend arrows and sparklines render; clicking a card shows detail list
- [x] 10.8 All existing bot commands unaffected
