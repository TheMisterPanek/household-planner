...
- [ ] 1.1 Register `MealRepository` as `AddScoped`
- [ ] 1.2 Register `MealIngredientRepository` as `AddScoped`
- [ ] 1.3 Register `MealStepRepository` as `AddScoped`
- [ ] 1.4 Register `DayMealsRepository` as `AddScoped`
- [ ] 1.5 Register `MealMergeService` as `AddScoped`
- [ ] 1.6 Register `IAiQueryService` → `AiQueryService` as `AddScoped`
- [ ] 1.7 Register `IOpenRouterClient` → `OpenRouterClient` via `AddHttpClient<IOpenRouterClient, OpenRouterClient>()`
- [ ] 1.8 Register `ConversationHistoryService` as `AddSingleton` if not already present

---

## 2. `/meals` — Meals Library Page

- [ ] 2.1 Create `Components/Pages/Meals.razor` inheriting `AuthenticatedPageBase`:
  - Load meal list via `MealRepository.GetAllAsync(groupId)`
  - Two-column layout: meal list sidebar (left), detail panel (right)
- [ ] 2.2 Meal list sidebar:
...
- [ ] 2.4 Detail panel — ingredients table:
...
- [ ] 3.1 Clicking a meal in the sidebar loads its ingredients and steps in the detail panel
...
- [ ] 3.5 [+ Add] ingredient → `UpsertAsync` called with correct args
...
- [ ] 4.1 Create `Components/Pages/WeekPlan.razor` inheriting `AuthenticatedPageBase`:
...
- [ ] 5.1 Page renders 7 rows with correct day names
- [ ] 5.2 Existing plan assignments pre-populate the selects correctly
- [ ] 5.3 Selecting a meal calls `UpsertAsync` with correct `dayOfWeek` and `mealId`
- [ ] 5.4 Selecting "— Not set —" calls `ClearAsync` with correct `dayOfWeek`

---

## 6. `/settings` — Settings Page

- [ ] 6.1 Create `Components/Pages/Settings.razor` inheriting `AuthenticatedPageBase`:
...
- [ ] 6.2 If `SetExpiryThresholdAsync` does not exist on `PreferenceRepository`, add it (stores key `expiry_threshold_days`, follows existing preference-storage pattern)
...
- [ ] 7.2 Clicking Save calls `SetLanguageAsync` and `SetExpiryThresholdAsync` with correct values
- [ ] 7.3 Success message appears after save

---

## 8. `/ai` — AI Chat Page

- [ ] 8.1 Create `Components/Pages/AiChat.razor` inheriting `AuthenticatedPageBase`:
...
- [ ] 8.2 If `OPENROUTER_API_KEY` is not set, `AiQueryService` will throw; ensure the error banner handles this gracefully with a user-friendly message

---

## 9. Unit Tests — `/ai`

- [ ] 9.1 Submitting a message appends it to the list and calls `QueryAsync`
- [ ] 9.2 Response from `QueryAsync` appended as bot message
- [ ] 9.3 Exception from `QueryAsync` shows error banner; does not clear user input; `_thinking` reset to false

---

## 10. NavMenu Update

- [ ] 10.1 Verify nav links for `/meals`, `/week`, `/settings`, `/ai` in `NavMenu.razor` point to the correct routes
