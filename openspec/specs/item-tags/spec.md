# item-tags Specification

## Purpose
Let items on the shared shopping list carry any number of tags (many-to-many, replacing the single-category model from `product-tags`), suggest frequently-used tags per group, carry tags into purchase history, and support filtering `/list` by one or more tags.
